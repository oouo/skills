...
2. `meta/SOURCES.tsv` mapping sequence, filename, video ID, theme, source file,
   and version note; use the six-column schema below.
3. `meta/SHA256SUMS` for every final cover.
4. `meta/TOPBAR-TEXT.txt` with one final label per cover.
5. `meta/TOPBAR-FONT.json` generated by
   `scripts/resolve-top-bar-font.py --format manifest`, recording the exact
   contract ID, source, asset, SHA-256, approval-record hash, and source-stroke value.
   Reject a v1/4px manifest under bundled v2; do not upgrade old records by relabeling them.
6. `meta/MAIN-TITLES.tsv` with one final title mode per cover and no
   `artwork-candidate` rows.
7. A full-series montage and top-bar montage.
8. Main-title, subject-middle, and bottom/edge QA montages.
9. Exact-cell and three-column profile previews in intended order.
10. SHA-256 manifests for QA and profile previews.
11. A package-local review record naming the exact package and status.
12. For reference-guided hand-brushed titles, the approved canonical RGBA title
    layer, its SHA-256, style contract ID, style-reference provenance, and exact
    user approval.
13. For `original-illustration`, the source-frame attempt and review, exact user
...
### Manifest schemas and paths

Write real tab-separated fields with the headers below; keep every field nonempty.
The example uses escaped tabs only to make the delimiters visible:

```python
sources_header = "sequence\tfilename\tvideo_id\ttheme\tsource\tnote\n"
titles_header = "sequence\tfilename\tmode\tartifact\tartifact_sha256\tapproval_record\n"
deterministic_row = "01\t01-cover.png\tdeterministic-font\tnone\tnone\tnot-applicable\n"
```

- Use two-digit sequences matching the numbered PNG names, with one row per cover.
- In `SOURCES.tsv`, `source` is relative to `PROJECT_ROOT`, stays inside it, and
  identifies the final assembled source PNG that is byte-identical to the packaged
  cover. Original videos, keyframes, and generated plates belong in the evidence
  chain; they are not substitutes for this final-file hash comparison.
- For `locked-artwork`, use `meta/title-artwork/<name>.png` for the canonical
  full-canvas sRGB RGBA layer, its SHA-256 for `artifact_sha256`, and
  `meta/title-approvals/<name>.txt` for the approval record naming that exact hash.
  Both paths are relative to the package. For `deterministic-font`, use the three
  literal values shown above; `artwork-candidate` always fails.
- Generate `TOPBAR-FONT.json` with the resolver in the same environment used for
  rendering. Preserve any approved `SOURCE_STROKE_W` override during verification.
- Cover hashes name root-level PNGs relative to the package. Put at least one
  `*SHA256SUMS` file in `qa/`; its entries resolve relative to `qa/`.

`check-cover-release.sh` checks these manifests, packaged file properties, font
integrity/glyph coverage, title state, and QA hashes. It does not prove that the
recorded font/stroke produced the image pixels, that locked artwork was composited
unchanged, or that a human approved the result. Run the remaining checks below.
...
- `meta/TOPBAR-FONT.json` exactly matches the resolved skill contract
- bundled `LXGWWenKai-Medium.ttf` exists and matches the locked SHA-256, or an
  explicit override has a hash-bound approval record
- missing or changed font data returns `HOLD`; no host-system font substitution
...
- `original-illustration` records the original video evidence, first source-frame
  attempt, its review status, and exact user-feedback trigger while recording the
  approved generated plate as the visual origin; weak frames without user feedback
